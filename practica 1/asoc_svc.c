...
asociacionprog_1(struct svc_req *rqstp, register SVCXPRT *transp)
{
	...
	case OBTENERASOCIACION:
		...
		_xdr_result = (xdrproc_t) xdr_ResultadoBusqueda;
	...
	case ENUMERAR:
		...
		_xdr_result = (xdrproc_t) xdr_ResultadoEnumerar;
...
main (int argc, char **argv)
{
	...
	pmap_unset (ASOCIACIONPROG, ASOCIACIONVERS);
	...
	if (!svc_register(transp, ASOCIACIONPROG, ASOCIACIONVERS, asociacionprog_1, IPPROTO_UDP)) {
		fprintf (stderr, "%s", "unable to register (ASOCIACIONPROG, ASOCIACIONVERS, udp).");
		exit(1);
	}
	...
	if (!svc_register(transp, ASOCIACIONPROG, ASOCIACIONVERS, asociacionprog_1, IPPROTO_TCP)) {
		fprintf (stderr, "%s", "unable to register (ASOCIACIONPROG, ASOCIACIONVERS, tcp).");
		exit(1);
	}
